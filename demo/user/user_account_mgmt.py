...
from backend.src.services.user_service import user_update_user_password, user_update_email, user_update_phone, user_view_account
    ...
    with c2:
        if st.button("Confirm Password Change", use_container_width=True):
            try:
                ...
                user_update_user_password(user_id = target_user_id, new_pword = new_password)
        ...
        if st.button("Confirm Change", use_container_width=True):
            try:
                ...
                if email is not None and email != "":
                    user_update_email(user_id = target_user_id, u_email = email)

                if phone_number is not None and phone_number != "":
                    user_update_phone(user_id = target_user_id, u_phone = phone_number)

                updated_account = user_view_account(user_id = st.session_state.get("user_id"))
...
with col2:
    st.page_link(
        ...
        label="⬅️ Back to User Dashboard",
